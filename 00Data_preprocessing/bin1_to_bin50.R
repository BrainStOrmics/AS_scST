LoadBGI_Spatial <- function(filename,
                            outdir = getwd(),
                            bin_data = T,
                            bin_size = 50,####You can change the size of the rectangle bulk which you need
                            cell_mask = F,
                            area_mask = F,
                            save_as = "rds",
                            pro_name = "Spatial",
    ...
    if (bin_data) {
        ...
        dat$x <- trunc((dat$x - 0) / bin_size + 1)
        dat$y <- trunc((dat$y - 0) / bin_size + 1)
    ...
    if (cell_mask) {
        ...
        if (UMI_GreyScale_Image) {
            ...
            tissue_lowres_image_r <- raster::raster(tissue_lowres_image)
            tissue_lowres_image_r <- raster::writeRaster(tissue_lowres_image_r, file.path(outdir, paste0(pro_name, "_UMIGreyScaleFakeIMG.tiff")), overwrite=T)
        } else tissue_lowres_image <- matrix(0, max(bin.coor.bkp$y), max(bin.coor.bkp$x))
    ...
    if (save_as == "h5ad") {
        ...
        names(coordinates) <- assay

        ad <- anndata::AnnData(X = object@assays$Spatial@counts, obs = genes, var = cells, varm = coordinates)
        ad <- ad$T

        DUMP <- write_h5ad.fix(ad, file.path(outdir, paste0(pro_name, "_SquidpySpatialObj.h5ad")))
    }
